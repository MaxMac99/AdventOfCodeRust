...
fn has_surrounding(found: &Match, line: &String, previous: &Option<String>, next: &Option<String>) -> bool {
    ...
    let start: i64 = found.start() as i64;
    let end: i64 = found.end() as i64;
    if start > 0 {
        chars.push(line.chars().nth((start - 1) as usize).unwrap())
    }
    if end < line.len() as i64 {
...
fn append_to_chars_in_range(previous: &Option<String>, chars: &mut Vec<char>, start: i64, end: i64) {
